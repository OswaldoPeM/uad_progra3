#pragma once
#ifdef VECTOR_DLL
#define MATH_HELPER extern "C" __declspec(dllexport)
#else
#define MATH_HELPER extern "C" __declspec(dllexport)
#endif
#include <math.h>
...
namespace  MathHelper
{
	...
	inline static Matrix4 ModelMatrix(float angleInRadians, CVector3_Dll translation)
	{
		float cosine = cosf(angleInRadians);
		float sine = sinf(angleInRadians);
	...
	inline static Matrix4 ViewMatrix(CVector3_Dll camEyePos, CVector3_Dll camLookAt, CVector3_Dll camUpVec)
	{
		// Calculate forward vector from target to eye
		CVector3_Dll forward = camEyePos - camLookAt;
		forward.normalize();

		// Calculate left vector
		CVector3_Dll	left = camUpVec.cross(forward);
		left.normalize();

		// Recalculate the orthonormal up vector
		CVector3_Dll up = forward.cross(left);
